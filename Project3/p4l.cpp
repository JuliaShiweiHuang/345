...
11/20/2015 -
Lithia Helmreich
Purpose of this program: This simulation implements the first-fit memory allocation scheme given a file with processes to allocate and deallocate, and returns the resulting memory allocation of implemented scheme as well as statistics.
There is one constant parameter in the file which can be changed, this is the memorySize and dictates how large the simulation's ram should be. The only input parameter for the simulation is the name of the filename which contains the processes, each on a line, in the order that events should be handled. Each process (line of the input file) is either in the format <A> <ID> <SIZE> where <A> = "A" meaning the process will be allocated memory, <ID> is the process ID, and <SIZE> is the size of the process, or in the format <D> <ID> where <D> = "D" meaning the processes will be deallocated from memory, and <ID> is the process ID to deallocate. Deallocation of a process can only occur after a process has already been allocated (ie the line "D 42" must occur somewhere after the line "A 42 23222").
If memory is full when a process is allocated, the process will be skipped. A deallocation order for a process that was skipped will do nothing.
Upon finishing allocating processes according to the first-fit algorithm, the output will be as follows (sample outputs provided):
(1): A memory table listing the processes currently in memory, their start and end addresses, and the size of the process. A free block (no process in that part of memory) will be listed as having a process ID of -1.
            MEMORY TABLE
------------------------------------
PID     Start       End      Size
------------------------------------
...
15    131400    182399     51000
17    182400    187399      5000
-1    187400    189999      2600
------------------------------------
(2) The list of processes that were skipped over because there was no room for them in memory at the time of allocation.
------------------------------------
Processes that did not fit in memory:
PID: 9
PID: 15
PID: 29
------------------------------------
(3) Summary statistics listing the current sum of memory in use across all processes, the amount of free memory remaining in memory, the number of free memory blocks remaining, and the average size of one of those free blocks (all sizes in bytes).
------------------------------------
Memory in use: 3423 bytes
------------------------------------
Free memory remaining: 54312 bytes
------------------------------------
Remaining free memory blocks: 53
------------------------------------
Avg. size of free blocks: 323 bytes
 */
